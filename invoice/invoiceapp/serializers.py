...
class InvoiceItemSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if item_type == 'product' and not product:
            raise serializers.ValidationError("Product ID is required for product-type items.")
        if item_type == 'service' and not name:
            raise serializers.ValidationError("Name is required for service-type items.")
        return data
